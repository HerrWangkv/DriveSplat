...
class DirectDiffusionControlNetPipeline(
    ...
    def __call__(
        ...
        cond_image: PipelineImageInput = None,
        ...
        **kwargs,
        ...
        Args:
            rgb_in (`torch.FloatTensor`, *optional*):
                The input RGB image for depth generation.
            prompt (`str` or `List[str]`, *optional*):
                The prompt or prompts to guide image generation. If not defined, you need to pass `prompt_embeds`.
            cond_image (`torch.Tensor`, `PIL.Image.Image`, `np.ndarray`, `List[torch.Tensor]`, `List[PIL.Image.Image]`, `List[np.ndarray]`,:
                    `List[List[torch.Tensor]]`, `List[List[np.ndarray]]` or `List[List[PIL.Image.Image]]`):
                The ControlNet input condition to provide guidance to the `unet` for generation. If the type is
                specified as `torch.Tensor`, it is passed to ControlNet as is. `PIL.Image.Image` can also be accepted
                as an image. The dimensions of the output image defaults to `cond_image`'s dimensions. If height and/or
                width are passed, `cond_image` is resized accordingly. If multiple ControlNets are specified in `init`,
        ...
        if isinstance(controlnet, ControlNetModel):
            ...
            assert height, width == cond_image.shape[-2:]
        elif isinstance(controlnet, MultiControlNetModel):
            images = []

            # Nested lists as ControlNet condition
            if isinstance(cond_image[0], list):
                # Transpose the nested image list
                cond_image = [list(t) for t in zip(*cond_image)]
            ...
            cond_image = images
            assert height, width == cond_image[0].shape[-2:]
        else:
            assert False
...
class SDaIGControlNetPipeline(DirectDiffusionControlNetPipeline):
    ...
    def __call__(
        ...
        **kwargs,
        ...
        if match_input_res:
            resample_method = (
                get_pil_resample_method("nearest")
                if output_type == "pil"
